...
public class QuadraticProbingHashTable {
	...
	public QuadraticProbingHashTable(int size) {
		...
		currentSize = nonNull = 0;
	}

	public int locatePos(int key) {

		int i = 0;
		int h = hash(key, HashTable.length);
		int currentPos = h + i * i;

		while (HashTable[currentPos] != null && HashTable[currentPos].isActive && HashTable[currentPos].element != key)

		{

			i++;
			currentPos = (h + i * i) % HashTable.length;
	...
	// Hint: first check the load factor after add - if the size is beyond
	// rehash first!
	public void insert(int x) {
		...
		if (HashTable[pos] != null && HashTable[pos].element == x && HashTable[pos].isActive) {
			return;
		}
		...
		double load_factor = ((double) nonNull) / HashTable.length;

		if (load_factor >= LOAD_FACTOR) {
			rehash();
		}

		// when you remove dont change the non null, set element to non active
		// and decrement size
	...
	// remember you are not going to remove the element from the hash table
	// (physcially)
	...
	public static void main(String[] args) {
		...
		System.out.println("Assignme0nt Testing Done!!!");
