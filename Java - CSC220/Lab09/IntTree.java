...
public class IntTree {
	...
	// as a string, from left to right, one per line . We will use the
	// convention that the overall root is at level 1, its
	// children are at level 2, and so on. If there are no values at the level,
	// your method should return an
	...
	public String getLevel(int level) {
		...
		if (level > 0) {
			return getLevel(level, overallRoot);
		}
	...
	private String getLevel(int level, IntTreeNode node) {
		...
		if (level == 1) {
			...
			return getLevel(level - 1, node.left) + getLevel(level - 1, node.right);
	...
	// branches in a tree. An empty tree is
	// considered to have one empty branch (the tree itself). For nonempty
	// trees, your methods should count the
	// total number of empty branches among the nodes of the tree. A leaf node
	// has two empty branches, a node
	// with one nonempty child has one empty branch, and a node with two
	// nonempty children has no empty branches.
	...
	// parenthesized String that has three elements separately by commas: the
	// data at the root, a string
	// representation of the left subtree, and then a string representation of
	// the right subtree.
	// See the instruction for example output.
	...
	private String toString(IntTreeNode node) {
		...
		if (node == null) {
			...
			{
				tree = "(" + tree + ", " + toString(node.left);

				tree = tree + ", " + toString(node.right) + ")";
	...
	// Write a method called makePerfect that adds nodes until the binary tree
	// is a perfect tree. A perfect
	// binary tree is one where all leaves are at the same level. A///nother way
	// of
	// thinking of it is that you
	// are adding dummy nodes to the tree until every path from the root to a
	// leaf is the same length. A
	// perfect tree's shape is triangular and every branch node has exactly two
	// children, and all of the
	// leaves are at the same level. Each new node you add to the tree should
	// store the value 0.

	public void makePerfect() {

		makePerfect(height(overallRoot), overallRoot);

	}

	private IntTreeNode makePerfect(int level, IntTreeNode node) {
		...
		if (node == null) {
			...
			node.left = makePerfect(level - 1, node.left);
			node.right = makePerfect(level - 1, node.right);
	...
	public int height(IntTreeNode node) {
	...
	public int counthelper(IntTreeNode root) {
		...
		if (root.left == null && root.right == null) {
			...
			return 1 + counthelper(root.right);
		}

		else if (root.right == null) {
			return 1 + counthelper(root.left);
		}

		else
			return counthelper(root.right) + counthelper(root.left);
	}
